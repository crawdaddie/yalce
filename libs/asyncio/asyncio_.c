...
void event_loop_register_op(EventLoop *loop, GenericOp *op);
...
void *accept_promise(int server_fd) {
  ...
  *p = (AcceptPromise){
  ...
  event_loop_register_op(_loop, p);
  return p;
}
...
void *io_accept(int server_fd, int *client_fd, void *cor) {
  if (!_loop) {
    fprintf(stderr, "event loop error: no current event loop running\n");
    return NULL;
  }
  printf("io accept\n");
  return NULL;
...
void event_loop_register_coroutine(EventLoop *loop, void *cor) {}
