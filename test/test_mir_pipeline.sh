...
IMPORTED_PAYLOAD_CTOR_SRC="$SCRIPT_DIR/mir_scripts/imported_payload_ctor_pipeline.ylc"
TAIL_CLEANUP_SRC="$SCRIPT_DIR/mir_scripts/tail_call_cleanup_probe.ylc"
TAIL_IMPORTED_CTOR_SRC="$SCRIPT_DIR/mir_scripts/tail_call_imported_ctor_pipeline.ylc"
TAIL_MODULE_SELF_SRC="$SCRIPT_DIR/mir_scripts/tail_call_module_self_pipeline.ylc"
TAIL_VOID_SRC="$SCRIPT_DIR/mir_scripts/tail_call_void_pipeline.ylc"
...
TAIL_CLEANUP_MIR="$ARTIFACT_DIR/tail_call_cleanup_probe.mir"
TAIL_CLEANUP_LLVM_IR="$ARTIFACT_DIR/tail_call_cleanup_probe.ll"
TAIL_VOID_LLVM_IR="$ARTIFACT_DIR/tail_call_void_pipeline.ll"
TAIL_IMPORTED_CTOR_MIR="$ARTIFACT_DIR/tail_call_imported_ctor_pipeline.mir"
TAIL_MODULE_SELF_MIR="$ARTIFACT_DIR/tail_call_module_self_pipeline.mir"
IMPORTED_PAYLOAD_CTOR_MIR="$ARTIFACT_DIR/imported_payload_ctor_pipeline.mir"
...
run_dump_mir "$IMPORTED_PAYLOAD_CTOR_SRC" "$IMPORTED_PAYLOAD_CTOR_MIR"
run_dump_mir "$TAIL_CLEANUP_SRC" "$TAIL_CLEANUP_MIR"
run_dump_mir "$TAIL_IMPORTED_CTOR_SRC" "$TAIL_IMPORTED_CTOR_MIR"
run_dump_mir "$TAIL_MODULE_SELF_SRC" "$TAIL_MODULE_SELF_MIR"
...
run_dump_llvm_pre "$TAIL_CLEANUP_SRC" "$TAIL_CLEANUP_LLVM_IR"
run_dump_llvm_pre "$TAIL_VOID_SRC" "$TAIL_VOID_LLVM_IR"
...
TAIL_CLEANUP_RUN_VM_BODY="$ARTIFACT_DIR/tail_cleanup_run_vm.mir"
TAIL_IMPORTED_CTOR_RUN_BODY="$ARTIFACT_DIR/tail_imported_ctor_run.mir"
TAIL_MODULE_SELF_STEP_BODY="$ARTIFACT_DIR/tail_module_self_step.mir"
IMPORTED_PAYLOAD_ALL_DONE_BODY="$ARTIFACT_DIR/imported_payload_all_done.mir"
...
extract_function "$TAIL_CLEANUP_MIR" "run_vm" "$TAIL_CLEANUP_RUN_VM_BODY"
extract_function "$TAIL_IMPORTED_CTOR_MIR" "run" "$TAIL_IMPORTED_CTOR_RUN_BODY"
extract_function "$TAIL_MODULE_SELF_MIR" "tail_call_module_self_dep.step" "$TAIL_MODULE_SELF_STEP_BODY"
extract_function "$IMPORTED_PAYLOAD_CTOR_MIR" "all_done" "$IMPORTED_PAYLOAD_ALL_DONE_BODY"
...
assert_order "$TAIL_RETURN_ENTRY_BODY" "tail-recursive entry should pass the returned array into the tail call" \
  ...
  'return %[0-9]+'
assert_not_contains "$TAIL_CLEANUP_RUN_VM_BODY" '^  bb[0-9]+ match\.cont:' "tail-position match should not lower through a value continuation"
assert_contains "$TAIL_CLEANUP_RUN_VM_BODY" 'tail_call %[0-9]+\(%2\).*%2:value/consume#0' "tail-position self call should transfer the recursive state"
assert_contains "$TAIL_VOID_LLVM_IR" 'tail call void @run\(i8 1\)' "void tail calls should lower as LLVM tail calls"
assert_not_contains "$TAIL_VOID_LLVM_IR" 'ret void <badref>' "void tail calls should return with ret void"
assert_contains "$TAIL_IMPORTED_CTOR_RUN_BODY" 'construct\.tuple' "qualified imported record constructor should lower to tuple construction"
assert_contains "$TAIL_IMPORTED_CTOR_RUN_BODY" 'tail_call %[0-9]+\(%[0-9]+\)' "qualified constructor in tail argument should preserve self tail call"
assert_contains "$TAIL_MODULE_SELF_STEP_BODY" 'tail_call %[0-9]+\(%[0-9]+\)' "module-local self call should be recognized through fn_ref identity"
assert_contains "$IMPORTED_PAYLOAD_ALL_DONE_BODY" 'tag_eq %[0-9]+, Done#1' "qualified imported variants should bind inside tuple patterns"
