...
payload+="$(make_msg '{"jsonrpc":"2.0","id":15,"method":"shutdown","params":null}')"

output="$(printf '%s' "$payload" | YLC_BASE_DIR="$ROOT_DIR" "$LSP_SERVER" | tr -d '\r')"

echo "$output"

grep -q '"id":1,"result":{"capabilities":{"textDocumentSync":{"openClose":true,"change":1},"documentSymbolProvider":true,"selectionRangeProvider":true,"hoverProvider":true,"definitionProvider":true,"renameProvider":true,"completionProvider":{"resolveProvider":false' <<<"$output"
...
grep -q '"id":15,"result":null' <<<"$output"
