...
int main() {
  ...
  status &= test_parse("`here's a printed "
                       "version of {x} and {y}\\n`",
                       "(fmt [\"here's a printed version of \", (str x), \" "
                       "and \", (str y), \"\n\"])");

  status &= test_parse(
      "`here's a printed "
      "version of {1 + 2000} and {y 1 2}`",
      "(fmt [\"here's a printed version of \", (str ((+ 1) 2000)), \" and "
      "\", (str ((y 1) 2))])");
  ...
  status &= test_parse("x@1", "(. x 1)");

  status &= test_parse("x.f 1 2 == 3", "((== (((. x f) 1) 2)) 3)");
  status &= test_parse("x[10]", "((array_at x) 10)");
  status &= test_parse("x[10] := 1", "(((array_set x) 10) 1)");

  status &= test_parse("if x then 1 else 2", "(match x with\n"
                                             "\ttrue -> 1\n"
                                             "\tfalse -> 2\n"
                                             ")");

  status &= test_parse("if x then ()", "(match x with\n"
                                       "\ttrue -> ()\n"
                                       ")");
