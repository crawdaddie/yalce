...
static void draw_parameter_sliders(VSTEditorState *state, SDL_Renderer *renderer);
static int handle_mouse_param_interaction(VSTEditorState *state, int x, int y, bool is_down);

static SDL_Renderer *vst_renderer(VSTEditorState *state, SDL_Renderer *renderer);
...
int create_vst_view(char *plugin_handle) {
    ...
    for (int i = 0; i < state->param_count; i++) {
        ...
        // Get parameter name
        if (vst_get_parameter_name(plugin, i, state->param_names[i], MAX_PARAM_NAME_LENGTH) != VST_ERR_OK) {
        ...
        printf("Parameter %d: %s = %.2f\n", i, state->param_names[i], state->param_values[i]);
    }
...
static SDL_Renderer *vst_renderer(VSTEditorState *state, SDL_Renderer *renderer) {
    ...
    if (state->param_count > MAX_VISIBLE_PARAMS) {
        ...
        if (state->scroll_offset + MAX_VISIBLE_PARAMS < state->param_count) {
            ...
            SDL_RenderDrawLine(renderer, x_center, y_bottom, x_center - 10, y_bottom - 10);
            SDL_RenderDrawLine(renderer, x_center, y_bottom, x_center + 10, y_bottom - 10);
            SDL_RenderDrawLine(renderer, x_center - 10, y_bottom - 10, x_center + 10, y_bottom - 10);
        }
    }
    ...
    switch (event->type) {
        case SDL_MOUSEBUTTONDOWN:
            if (event->button.button == SDL_BUTTON_LEFT) {
                ...
                int param_idx = handle_mouse_param_interaction(state, event->button.x, event->button.y, true);
                if (param_idx >= 0) {
        ...
        case SDL_MOUSEMOTION:
            if (state->dragging_param >= 0) {
                ...
                normalized_pos = fmax(0.0f, fmin(1.0f, normalized_pos)); // Clamp to 0.0-1.0
...
static void draw_parameter_sliders(VSTEditorState *state, SDL_Renderer *renderer) {
    ...
    int display_count = fmin(MAX_VISIBLE_PARAMS, state->param_count - state->scroll_offset);
    ...
    for (int i = 0; i < display_count; i++) {
        ...
        // Draw value as text
        char value_str[16];
        snprintf(value_str, sizeof(value_str), "%.2f", state->param_values[param_idx]);
        render_text(value_str, slider_x + SLIDER_WIDTH + 10, y_pos, renderer, value_color);
        
        // Draw display value if available (using vst_get_parameter_display)
        char display_str[32] = {0};
        if (vst_get_parameter_display(state->plugin, param_idx, display_str, sizeof(display_str)) == VST_ERR_OK && display_str[0] != '\0') {
            // Get parameter label too
            char label_str[16] = {0};
            vst_get_parameter_label(state->plugin, param_idx, label_str, sizeof(label_str));
            
            // Combine display and label
            char formatted_value[48];
            if (label_str[0] != '\0') {
                snprintf(formatted_value, sizeof(formatted_value), "%s %s", display_str, label_str);
            } else {
                snprintf(formatted_value, sizeof(formatted_value), "%s", display_str);
            }
            
            render_text(formatted_value, slider_x + SLIDER_WIDTH + 70, y_pos, renderer, value_color);
        }
...
static int handle_mouse_param_interaction(VSTEditorState *state, int x, int y, bool is_down) {
    ...
    if (x >= slider_x && x <= slider_x + SLIDER_WIDTH) {
        ...
        for (int i = 0; i < fmin(MAX_VISIBLE_PARAMS, state->param_count - state->scroll_offset); i++) {
            if (y >= y_pos && y < y_pos + SLIDER_HEIGHT) {
                return i; // Return visible parameter index (not the actual parameter index)
            }
            y_pos += SLIDER_SPACING;
        }
    }
