...
Type next_tvar() { return (Type){T_VAR, {.T_VAR = type_var_counter++}}; }
...
static Type *infer_void_arg_lambda(TypeEnv **env, Ast *ast) {
  ...
  infer(&new_env, ast->data.AST_LAMBDA.body);

  // Unify the return type with the body type
  unify(&ret_var, &ast->data.AST_LAMBDA.body->md);
...
int infer(TypeEnv **env, Ast *ast) {
  ...
  case AST_BINOP: {
    ...
    int ltr = infer(env, ast->data.AST_BINOP.left);
    int rtr = infer(env, ast->data.AST_BINOP.right);
  ...
  case AST_IDENTIFIER: {
    ...
    if (!type) {
      Type *t = get_builtin_type(ast->data.AST_IDENTIFIER.value);
      if (!t) {
