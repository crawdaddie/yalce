...
ht *mir_durable_builtins_create(void) {
  ...
  ht_init_with_allocator(table, (ht_allocator){.alloc = mir_ht_alloc_persistent,
                                               .free = mir_ht_free_persistent,
                                               .ctx = NULL});
  return table;
}
...
static const char *mir_scope_strdup(MirCtx *ctx, const char *str) {
  if (!str) {
    return NULL;
  }
  MirArena *arena = mir_scope_arena(ctx);
...
static const char *mir_import_name_from_path(MirArena *arena,
  ...
  static const char suffix[] = ".ylc";
  size_t suffix_len = sizeof(suffix) - 1;
  if (len > suffix_len && strcmp(base + len - suffix_len, suffix) == 0) {
    len -= suffix_len;
  }
  return mir_arena_strndup(arena, base, len);
}

// Resolve the durable arena for top-level function bodies. Walks to the
// root frame and returns its durable_arena (set by the REPL session).
// Returns NULL when there is no durable arena (e.g. one-shot `mir()`
// compilation), in which case functions allocate from the program arena.
static MirArena *mir_durable_arena(MirCtx *ctx) {
  if (!ctx) {
    return NULL;
  }
  for (MirStackFrame *frame = ctx->frame; frame; frame = frame->next) {
    if (!frame->next) {
      return frame->durable_arena;
    }
  }
  return NULL;
}
...
// back to a module-local per-name LLVM global.
static int mir_durable_alloc_global_slot(MirCtx *ctx) {
  if (!ctx) {
    return -1;
  }
  for (MirStackFrame *frame = ctx->frame; frame; frame = frame->next) {
    if (!frame->next) {
      if (frame->durable_arena) {
        return frame->global_slot_counter++;
      }
      return -1;
    }
  }
...
MirSymbol *mir_ctx_lookup_symbol(MirProgram *program, MirCtx *ctx,
  ...
  if (ctx) {
    for (MirStackFrame *frame = ctx->frame; frame; frame = frame->next) {
      ...
      // Skip MIR_SYMBOL_VALUE bound in a prior MirProgram: its as.value is
      // a program-relative id that's stale here. MIR_SYMBOL_FUNCTION is
      // now durable by pointer (MirFunction * survives across programs),
      // so it is NOT skipped. Name-keyed kinds (GLOBAL, MODULE, EXPR) are
      // durable and re-resolve in the current program. Falling through
      // lets the caller reach the name-based resolution paths
...
static MirFunction *mir_program_find_specialization(MirProgram *program,
                                                    MirFunction *source,
                                                    Type *type) {
  if (!program || !source || !type) {
    return NULL;
  }

  for (size_t i = 0; i < program->functions.len; i++) {
    MirFunction *fn = program->functions.items[i];
    if (fn && fn->specialization_of == source && fn->specialization_type &&
...
MirFunction *mir_program_add_function(MirProgram *program, const char *name,
                                      Type *type, Ast *origin) {
  return mir_program_add_function_arena(program, name, type, origin,
                                        program ? program->arena : NULL);
}
...
MirFunction *mir_program_add_function_arena(MirProgram *program,
  ...
  MirArena *fn_arena = alloc_arena ? alloc_arena : program->arena;
  MirFunction *fn =
      mir_arena_alloc(fn_arena, sizeof(MirFunction), MIR_ALIGNOF(MirFunction));
  if (!fn) {
    return NULL;
  }
  ...
  fn->arena = fn_arena;
  fn->name = mir_arena_strdup(fn_arena, name ? name : "<anonymous>");
  fn->type = type;
  fn->origin = origin;
  fn->specialization_of = NULL;
...
// Private emission helpers. Each one builds a single MIR_OP/MIR_EXTRACT/
// MIR_CONSTRUCT/MIR_CONST/MIR_CALL-like instruction from its operands and
...
MirValueId mir_emit_const(MirBuilder *builder, MirConst const_value, Type *type,
  ...
  instr.data.const_value = const_value;
...
// Initialize the common call-shaped payload shared by MIR_CALL, MIR_CORO_NEW,
// MIR_CORO_NEXT and MIR_CORO_RESET. The caller still owns specialization of
// the instr (callee_type, builtin, etc.) before appending.
void mir_emit_call_init(MirInstr *instr, MirValueId callee,
  ...
  instr->data.call.builtin = builtin;
  instr->data.call.callee_type = callee_type;
  instr->data.call.specialized_fn = NULL;
}
...
MirValueId mir_const_int(MirBuilder *builder, Type *type, Ast *origin,
                         int value) {
  return mir_emit_const(
      builder, (MirConst){.kind = MIR_CONST_KIND_INT, .as.int_value = value},
      type, origin);
}
...
MirValueId mir_const_uint64(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_const(
      builder,
      (MirConst){.kind = MIR_CONST_KIND_UINT64, .as.uint64_value = value}, type,
      origin);
}
...
MirValueId mir_const_float(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_const(
      builder,
      (MirConst){.kind = MIR_CONST_KIND_FLOAT, .as.float_value = value}, type,
      origin);
}
...
MirValueId mir_const_double(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_const(
      builder,
      (MirConst){.kind = MIR_CONST_KIND_DOUBLE, .as.double_value = value}, type,
      origin);
}
...
MirValueId mir_const_char(MirBuilder *builder, Type *type, Ast *origin,
                          char value) {
  return mir_emit_const(
      builder, (MirConst){.kind = MIR_CONST_KIND_CHAR, .as.char_value = value},
      type, origin);
}
...
MirValueId mir_const_bool(MirBuilder *builder, Type *type, Ast *origin,
                          bool value) {
  return mir_emit_const(
      builder, (MirConst){.kind = MIR_CONST_KIND_BOOL, .as.bool_value = value},
      type, origin);
}
...
MirValueId mir_const_void(MirBuilder *builder, Type *type, Ast *origin) {
  return mir_emit_const(builder, (MirConst){.kind = MIR_CONST_KIND_VOID}, type,
                        origin);
}

MirValueId mir_const_undef(MirBuilder *builder, Type *type, Ast *origin) {
  return mir_emit_const(builder, (MirConst){.kind = MIR_CONST_KIND_UNDEF}, type,
                        origin);
}
...
MirValueId mir_coro_next(MirBuilder *builder, Ast *origin, MirValueId coroutine,
  ...
  mir_emit_call_init(&instr, coroutine, NULL, coroutine_type);
  return mir_builder_append_instr(builder, instr);
}
...
MirValueId mir_tuple(MirBuilder *builder, Type *type, Ast *origin,
                     MirValueIdVec items) {
  return mir_emit_construct_items(builder, MIR_CONSTRUCT_TUPLE, type, origin,
                                  items);
}

MirValueId mir_tuple_get(MirBuilder *builder, Type *type, Ast *origin,
                         MirValueId tuple, size_t index) {
  return mir_emit_extract_field(builder, type, origin, tuple, index, NULL);
}
...
MirValueId mir_variant_tag(MirBuilder *builder, Ast *origin, MirValueId value) {
  return mir_emit_extract(builder, MIR_EXTRACT_VARIANT_TAG, &t_char, origin,
                          value);
}
...
MirValueId mir_list_empty(MirBuilder *builder, Type *type, Ast *origin) {
  ...
  return mir_emit_construct(builder, MIR_CONSTRUCT_LIST_EMPTY, type, origin);
}
...
MirValueId mir_list_cons(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_construct_ops2(builder, MIR_CONSTRUCT_LIST_CONS, type, origin,
                                 head, tail);
}

MirValueId mir_list_is_empty(MirBuilder *builder, Ast *origin,
                             MirValueId list) {
  return mir_emit_op1(builder, MIR_OP_KIND_LIST_IS_EMPTY, &t_bool, origin,
                      list);
}
...
MirValueId mir_list_head(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_extract(builder, MIR_EXTRACT_LIST_HEAD, type, origin, list);
}
...
MirValueId mir_list_tail(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_extract(builder, MIR_EXTRACT_LIST_TAIL, type, origin, list);
}
...
MirValueId mir_ptr_offset(MirBuilder *builder, Type *ptr_type, Ast *origin,
  ...
  return mir_emit_op2(builder, MIR_OP_KIND_PTR_OFFSET, ptr_type, origin, ptr,
                      index);
}
...
static MirValueId mir_ptr_load_with_kind(MirBuilder *builder, MirOpKind kind,
  ...
  return mir_emit_op1(builder, kind, type, origin, ptr);
}
...
MirValueId mir_ptr_store(MirBuilder *builder, Ast *origin, MirValueId ptr,
  ...
  return mir_emit_op2(builder, MIR_OP_KIND_STORE, &t_void, origin, ptr, value);
}
...
static MirValueId mir_closure_env(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_construct_items(builder, MIR_CONSTRUCT_CLOSURE_ENV, type,
                                  origin, fields);
}
...
static MirValueId mir_extract_field(MirBuilder *builder, Type *type,
  ...
  return mir_emit_extract_field(builder, type, origin, value, index, name);
}
...
static MirValueId mir_closure_fn(MirBuilder *builder, Type *type, Ast *origin,
  ...
  return mir_emit_extract(builder, MIR_EXTRACT_CLOSURE_FN, type, origin,
                          closure);
}
...
static MirValueId mir_closure_get_env(MirBuilder *builder, Type *type,
  ...
  return mir_emit_extract(builder, MIR_EXTRACT_CLOSURE_ENV, type, origin,
                          closure);
}
...
// Primitive arithmetic/equality wrappers. Each forwards to
// mir_primitive_binary (or mir_primitive_unary for lnot) with the matching
// MirPrimitiveOp; kept as named entry points so call sites read naturally and
// the result type is fixed for the comparison ops.
...
MirBuiltinSymbol *mir_program_register_builtin(MirProgram *program,
  ...
  // Builtins are a fixed core set. When a durable builtin cache is in use
  // (persistent REPL frame), allocate the MirBuiltinSymbol once (from the
  // durable arena) and reuse the same pointer across MirPrograms. This
  // keeps durable function bodies' MIR_CALL.builtin pointers valid after
  // their defining MirProgram is destroyed. Subsequent programs just
  // register the same durable pointer in their per-program ht.
  if (program->durable_builtins && program->durable_arena) {
    MirBuiltinSymbol *symbol = ht_get(program->durable_builtins, name);
    if (!symbol) {
      symbol = mir_arena_alloc(program->durable_arena, sizeof(MirBuiltinSymbol),
                               MIR_ALIGNOF(MirBuiltinSymbol));
      if (!symbol) {
        return NULL;
      }
      *symbol = (MirBuiltinSymbol){
          .name = mir_arena_strdup(program->durable_arena, name),
          .type = type,
          .handler = handler,
          .data = data,
...
static MirFunction *mir_specialize_fn_ref_instr(MirProgram *program,
  ...
  if (!program || !fn_ref || fn_ref->kind != MIR_FN_REF ||
      !fn_ref->data.fn_ref.fn || !specialized_type ||
  ...
  MirFunction *specialized =
      mir_program_find_specialization(program, target, specialized_type);
  if (!specialized) {
  ...
  if (specialized) {
    fn_ref->data.fn_ref.fn = specialized;
...
static MirInstr mir_clone_instr_with_subst(MirArena *arena, MirTypeSubst *subst,
  ...
  case MIR_CORO_RESET:
    ...
    clone.data.call.specialized_fn = NULL;
    ...
    for (size_t j = 0; j < source_block->instrs.len; j++) {
      ...
      if (clone.kind == MIR_CALL && !clone.data.call.builtin) {
        ...
        if (specialized) {
          ...
          clone.data.call.specialized_fn = specialized;
        }
...
static MirFunction *mir_materialize_call_specialization(MirBuilder *builder,
  ...
  MirFunction *source = callee_def->data.fn_ref.fn;
  if (!source) {
    return NULL;
  }
  ...
  MirFunction *existing = mir_program_find_specialization(
      builder->program, source, call->data.call.callee_type);
  if (existing) {
    return existing;
  }
...
static MirFunction *mir_materialize_builtin_function(MirProgram *program,
  ...
  mir_emit_call_init(&call, MIR_NO_VALUE, builtin, builtin->type);
...
mir_append_curried_inner_call(MirBuilder *wrapper_builder, Ast *origin,
  ...
  mir_emit_call_init(&call, callee_ref, NULL,
                     mir_fn_type_from_args(wrapper_builder->program->arena,
                                           operand_types, operand_count,
                                           result_type));
  ...
  if (specialized) {
    ...
    call.data.call.specialized_fn = specialized;
  }
...
static void mir_update_call_result_type(MirBuilder *builder, MirInstr *call,
  ...
  call->data.call.specialized_fn = NULL;
...
MirValueId mir_call_value(MirBuilder *builder, Type *type, Ast *origin,
  ...
  if (!builder || !builder->program || !builder->fn || callee == MIR_NO_VALUE ||
      !callee_type || callee_type->kind != T_FN || (argc > 0 && !args)) {
    return MIR_NO_VALUE;
  }

  MirInstr call = mir_make_instr(MIR_CALL, type, origin);
  MirArena *arena = builder->fn->arena;
  mir_emit_call_init(&call, callee, NULL, callee_type);
...
static MirValueId mir_global_load(MirBuilder *builder, Type *type, Ast *origin,
                                  const char *global_name, int global_slot) {
  ...
  instr.data.op.global_slot = global_slot;
  return mir_builder_append_instr(builder, instr);
}

static MirValueId mir_global_store(MirBuilder *builder, Type *type, Ast *origin,
                                   const char *global_name, int global_slot,
                                   MirValueId value) {
...
static MirSymbol *mir_open_import_member_symbol(
  ...
  MirSymbol *symbol =
      mir_symbol_new(mir_scope_arena(ctx), exported->kind, exported->type,
                     exported->origin, ctx->current_module);
  if (!symbol) {
    return NULL;
  }
  ...
  if (!symbol->module_path || !symbol->module_member_name) {
    return NULL;
  }
...
static MirFunction *mir_value_fn_ref_target(MirProgram *program,
  ...
  if (!instr || instr->kind != MIR_FN_REF || !instr->data.fn_ref.fn) {
    return NULL;
  }
  return instr->data.fn_ref.fn;
}
...
static bool mir_export_expr_binding(MirBuilder *builder, MirCtx *ctx,
  ...
  // this name (re-binding `x` in a later input), else allocate a fresh
  // slot from the root frame's counter. The slot indexes the
  // process-global storage array, so a store in one REPL input loads in
  // a later input via the same C array slot.
  int global_slot = -1;
  if (ctx && ctx->frame) {
    for (MirStackFrame *frame = ctx->frame; frame; frame = frame->next) {
      if (!frame->table) {
      ...
      MirSymbol *existing = ht_get(frame->table, name);
      if (existing && existing->kind == MIR_SYMBOL_GLOBAL &&
          existing->global_slot >= 0) {
        global_slot = existing->global_slot;
        break;
      }
    }
  }
  if (global_slot < 0) {
    global_slot = mir_durable_alloc_global_slot(ctx);
  } else if (ylc_config.debug_codegen) {
    // Hook point for future rebind handling: a durable global with this
    // name already exists, so this store reuses its storage slot rather
    // than allocating a new one. Logged only under --debug-codegen.
    fprintf(stderr,
            "[mir] rebind durable global `%s` (slot %d) in program gen %u\n",
            name ? name : "<anonymous>", global_slot,
            builder && builder->program ? builder->program->generation : 0u);
  }

  if (!global_name || mir_global_store(builder, type, expr, global_name,
                                       global_slot, value) == MIR_NO_VALUE) {
    return false;
  }
  ...
  symbol->global_slot = global_slot;
  return mir_bind_export_symbol(builder, ctx, name, symbol);
}
...
static MirValueId mir_lambda_value(MirBuilder *builder, Ast *expr, MirCtx *ctx,
  ...
  // Durable allocation: top-level non-capturing functions allocate their
  // MirFunction/blocks/instrs from the persistent arena held by the REPL
  // session, so the body survives mir_program_destroy and re-resolves
  // across inputs via the durable MIR_SYMBOL_FUNCTION (MirFunction *).
  // Closures are deferred (their env is program-relative); they keep
  // using the program arena.
...
static MirFunction *mir_build_module_init_fn(MirBuilder *parent_builder,
  ...
  const char *init_name =
      mir_arena_printf(parent_builder->program->arena, "$%s_module_init",
  ...
  if (!mir_compile_module_body(&init_builder, module_ast, module_ctx)) {
    return NULL;
  }

  if (init_builder.block && init_builder.block->term.kind == MIR_TERM_NONE) {
...
static bool mir_emit_call_to_fn(MirBuilder *builder, MirFunction *callee,
                                Ast *origin) {
  if (!builder || !builder->program || !callee || !origin) {
    return false;
  }

  MirValueId callee_ref = mir_fn_ref(builder, callee->type, origin, callee);
  if (callee_ref == MIR_NO_VALUE) {
    return false;
  }
  MirInstr call = mir_make_instr(MIR_CALL, &t_void, origin);
  mir_emit_call_init(&call, callee_ref, NULL, callee->type);
  mir_call_apply_callee_summary(builder, &call);
  return mir_builder_append_instr(builder, call) != MIR_NO_VALUE;
}

static MirValueId mir_specialized_module_binding_value(MirBuilder *builder,
                                                       Ast *ast, MirCtx *ctx) {
  ...
  const char *local_name = ast->data.AST_LET.binding->data.AST_IDENTIFIER.value;
  const char *module_name =
  ...
  MirFunction *init_fn =
      mir_build_module_init_fn(builder, module, generic->as.expr, &module_ctx);
  if (!init_fn || !mir_emit_call_to_fn(builder, init_fn, ast)) {
    return MIR_NO_VALUE;
  }
...
static bool mir_is_specialized_module_binding_expr(MirBuilder *builder,
                                                   Ast *expr, MirCtx *ctx) {
  if (!builder || !builder->program || !expr || expr->tag != AST_APPLICATION) {
    return false;
  }

  Ast *app =
      mir_application_flatten_if_saturated(builder->program->arena, expr);
  if (!app || app->tag != AST_APPLICATION) {
    return false;
  }
...
static MirValueId mir_module_binding_value(MirBuilder *builder, Ast *ast,
  ...
  MirSymbol *symbol =
      mir_symbol_new(mir_scope_arena(ctx), MIR_SYMBOL_MODULE, module_ast->type,
                     module_ast, ctx->current_module);
  if (!symbol) {
    return MIR_NO_VALUE;
  }
  ...
  MirFunction *init_fn =
      mir_build_module_init_fn(builder, module, module_ast, &module_ctx);
  if (!init_fn || !mir_emit_call_to_fn(builder, init_fn, ast)) {
    return MIR_NO_VALUE;
  }
...
static MirValueId mir_import_value(MirBuilder *builder, Ast *ast, MirCtx *ctx) {
  ...
  MirFunction *init_fn =
      mir_build_module_init_fn(builder, module, imported->ast, &module_ctx);
  if (!init_fn || !mir_emit_call_to_fn(builder, init_fn, ast)) {
    return MIR_NO_VALUE;
  }
  ...
  if (ast->data.AST_IMPORT.import_all) {
    ...
    for (bool cont = ht_next(&it); cont; cont = ht_next(&it)) {
      MirSymbol *opened = mir_open_import_member_symbol(
          builder, ctx, it.key, it.value, import_path, import_name);
      if (!opened || !mir_bind_export_symbol(builder, ctx, it.key, opened)) {
        return MIR_NO_VALUE;
      }
    }
  }
...
static MirValueId mir_let_value(MirBuilder *builder, Ast *ast, MirCtx *ctx) {
  ...
  if (expr && expr->tag == AST_MODULE) {
    if (ylc_config.test_mode && mir_is_test_binding(ast)) {
      return mir_const_void(builder, &t_void, ast);
    }
...
static MirValueId mir_let(MirBuilder *builder, Ast *ast, MirCtx *ctx) {
  ...
  // the old backend_llvm JISymbol, which carried both `val` and a
  ...
  // A durable top-level function was just bound via mir_bind_function_symbol
  // (MIR_SYMBOL_FUNCTION carrying the durable MirFunction *). Skip the
  // mir_bind_pattern value bind for it at top level, otherwise it would
  // clobber the durable FUNCTION symbol with a program-relative
  // MIR_SYMBOL_VALUE that's stale in the next input. Same-input
  // references resolve via mir_ctx_lookup_symbol -> mir_symbol_to_value
  // -> mir_fn_ref (the pointer). Closures are deferred (they keep the
  // value bind; cross-input closure use is a follow-up).
  ...
  if (!ast->data.AST_LET.in_expr) {
    if (durable_function_bound || exported_extern_function_bound) {
      ...
      // MIR_SYMBOL_VALUE that would clobber it. For externs this also
      // prevents module functions from capturing a fn_ref value that only
      // exists in the module init function.
      return value;
    }
...
static bool mir_imported_member_symbol_current(MirProgram *program,
  ...
  case MIR_SYMBOL_GLOBAL:
    return symbol->as.global_name != NULL;
  case MIR_SYMBOL_VALUE:
    return symbol->as.value != MIR_NO_VALUE;
  case MIR_SYMBOL_MODULE:
    return symbol->as.module != MIR_NO_MODULE &&
           symbol->as.module < program->modules.len;
  case MIR_SYMBOL_GENERIC_MODULE:
  case MIR_SYMBOL_EXPR:
  case MIR_SYMBOL_TYPE:
  case MIR_SYMBOL_CUSTOM:
...
static void mir_uniquify_rematerialized_module_functions(MirBuilder *builder,
                                                         size_t first_fn,
                                                         MirFunction *init_fn) {
  if (!builder || !builder->program || !builder->program->arena) {
    return;
  }
  ...
  for (size_t i = first_fn; i < builder->program->functions.len; i++) {
    ...
    fn->name = mir_arena_printf(builder->program->arena, "%s.repl.%u.%zu",
                                fn->name, builder->program->generation, i);
  }
}
...
static MirSymbol *mir_materialize_imported_module_symbol(MirBuilder *builder,
  ...
  if (symbol->generation == builder->program->generation &&
      symbol->as.module != MIR_NO_MODULE &&
      symbol->as.module < builder->program->modules.len) {
    return symbol;
  }

  YLCModule *imported = get_module(symbol->module_path);
  if (!imported) {
    return symbol;
  }
  if (!imported->ast || !imported->env) {
  ...
  module->path = mir_arena_strdup(builder->program->arena, symbol->module_path);

  symbol->as.module = module->id;
  symbol->type = module->type;
  symbol->origin = imported->ast;
  symbol->owner_module =
      ctx ? ctx->current_module : builder->program->root_module;
  symbol->generation = builder->program->generation;
  ...
  size_t first_module_fn = builder->program->functions.len;
  MirFunction *init_fn =
      mir_build_module_init_fn(builder, module, imported->ast, &module_ctx);
  if (init_fn) {
    init_fn->name = mir_arena_printf(
        builder->program->arena, "$%s_module_init.repl.%u",
        module->name ? module->name : "module", builder->program->generation);

    mir_uniquify_rematerialized_module_functions(builder, first_module_fn,
                                                 init_fn);

    mir_emit_call_to_fn(builder, init_fn, imported->ast);
  }
  return symbol;
}

static MirSymbol *mir_materialize_opened_import_member_symbol(
    MirBuilder *builder, MirCtx *ctx, const char *name, MirSymbol *symbol) {
  if (!builder || !builder->program || !symbol || !symbol->module_path ||
      !symbol->module_member_name) {
    return symbol;
  }

  if (mir_imported_member_symbol_current(builder->program, symbol)) {
    return symbol;
  }
  ...
  MirSymbol *member = mir_module_lookup_symbol(
      builder->program, module->id, symbol->module_member_name, false);
  if (!member) {
    return symbol;
  }

  const char *module_path = symbol->module_path;
  const char *member_name = symbol->module_member_name;
  const char *import_name = symbol->module_import_name;
  *symbol = *member;
  symbol->module_path = module_path;
  symbol->module_member_name = member_name;
  symbol->module_import_name = import_name;
  symbol->generation = builder->program->generation;
  return symbol;
}
...
MirSymbol *mir_resolve_ast_symbol(MirBuilder *builder, Ast *ast, MirCtx *ctx) {
  ...
  if (ast->tag == AST_IDENTIFIER) {
    ...
    MirSymbol *symbol = mir_ctx_lookup_symbol(builder->program, ctx, name);
    symbol = mir_materialize_imported_module_symbol(builder, ctx, name, symbol);
    return mir_materialize_opened_import_member_symbol(builder, ctx, name,
                                                       symbol);
  }
...
static MirValueId mir_identifier(MirBuilder *builder, Ast *ast, MirCtx *ctx) {
  ...
  if (ast && ast->tag == AST_IDENTIFIER) {
    MirSymbol *symbol = mir_resolve_ast_symbol(builder, ast, ctx);
...
static bool mir_populate_function_body(MirProgram *program, MirFunction *fn,
  ...
  // In interactive REPL mode, chain the function body's frame to the
  // parent frame so closures can resolve top-level globals (durable
  // MIR_SYMBOL_GLOBAL) via global_load and call durable functions, rather
  // than erroring "unresolved identifier". In one-shot script mode the
  // frame stays isolated (next=NULL): script top-level functions resolve
  // siblings by name (mir_program_find_function_by_name) and don't need
  // the parent chain, and chaining there can mis-resolve symbols.
  MirStackFrame *parent = NULL;
  if (ylc_config.interactive_mode && ctx && ctx->frame) {
    parent = ctx->frame;
  }
  mir_stack_frame_init(program->arena, &fn_table, &fn_frame, parent);
...
static void dump_instr(FILE *stream, const MirFunction *fn,
  ...
  case MIR_OP:
    ...
    case MIR_OP_KIND_GLOBAL_LOAD:
      fprintf(stream, " @%s",
              instr->data.op.global_name ? instr->data.op.global_name
                                         : "<global>");
      if (instr->type) {
        fputs(STYLE_DIM " : ", stream);
        print_type_to_stream(instr->type, stream);
...
static bool mir_build_test_top(MirProgram *program, Ast *prog, MirCtx *root_ctx,
  ...
  MirSymbol *symbol =
      mir_symbol_new(mir_scope_arena(root_ctx), MIR_SYMBOL_MODULE,
                     test_module->type, test_module, root_ctx->current_module);
  if (!symbol) {
    return false;
  }
  symbol->as.module = module->id;
  if (!mir_bind_export_symbol(builder, root_ctx, "test", symbol)) {
    return false;
  }

  ht module_table;
  MirStackFrame module_frame;
  mir_stack_frame_init(program->arena, &module_table, &module_frame, NULL);
  MirCtx module_ctx = {
      .env = root_ctx->env,
      .frame = &module_frame,
      .current_module = module->id,
  ...
  MirFunction *init_fn =
      mir_build_module_init_fn(builder, module, test_module, &module_ctx);
  if (!init_fn || !mir_emit_call_to_fn(builder, init_fn, test_module)) {
    mir_builder_set_unreachable_if_open(builder);
    return false;
  }

  MirModuleId test_module_id = module->id;
  ...
  for (AstList *item = stmts; item; item = item->next) {
    ...
    MirSymbol *member_symbol =
        mir_module_lookup_symbol(program, test_module_id, test_name, false);
    if (!member_symbol) {
      mir_builder_set_unreachable_if_open(builder);
      return false;
    }

    Type *sym_type = member_symbol->type ? member_symbol->type : &t_bool;
    MirValueId value = MIR_NO_VALUE;
    if (sym_type->kind == T_FN) {
      MirValueId callee =
          mir_symbol_to_value(builder, stmt, NULL, member_symbol, sym_type);
      if (callee == MIR_NO_VALUE) {
        mir_builder_set_unreachable_if_open(builder);
        return false;
      }
      MirInstr call = mir_make_instr(MIR_CALL, &t_bool, stmt);
      mir_emit_call_init(&call, callee, NULL, sym_type);
      mir_call_apply_callee_summary(builder, &call);
      value = mir_builder_append_instr(builder, call);
    } else {
      value = mir_symbol_to_value(builder, stmt, NULL, member_symbol, sym_type);
    }
    ...
    mir_emit_call_init(&report_call, report_result_ref, NULL,
                       report_result_type);
  ...
  mir_emit_call_init(&totals_call, report_totals_ref, NULL, report_totals_type);
...
// Register durable top-level functions (bound into the persistent root
// frame in a prior REPL input) into the current program's functions
// vector, assigning a fresh per-program id for declaration order. The
// durable MirFunction * is the identity; references resolve by pointer,
// so this is purely so lower_mir declares/lowers them in this program.
...
MirProgram *mir_build_program(MirArena *arena, Ast *prog_ast, MirCtx *ctx) {
  ...
  // Inherit the durable builtin cache + durable arena from the persistent
  // root frame so core builtins are allocated once and reused across
  // MirPrograms (durable function bodies hold MirBuiltinSymbol * pointers
  // that must stay valid after the defining program is freed).
  for (MirStackFrame *frame = build_ctx.frame; frame; frame = frame->next) {
    if (!frame->next) {
      program->durable_builtins = frame->durable_builtins;
      program->durable_arena = frame->durable_arena;
      break;
    }
  }
  ht_init_with_allocator(
  ...
  // Register durable top-level functions from the persistent root frame
  // into this program so they get declared/lowered here. Durable functions
  // (bound in a prior REPL input) live on the root frame; nested frames
  // have none.
  for (MirStackFrame *frame = build_ctx.frame; frame; frame = frame->next) {
    if (!frame->next) {
      mir_register_durable_functions(program, frame);
      break;
    }
  }
  ...
  MirFunction *top = mir_program_add_function(
      program, "$top",
      ylc_config.test_mode ? &t_bool : (prog_ast ? prog_ast->type : NULL),
      prog_ast);
  ...
  if (ylc_config.interactive_mode) {
    ...
    if (result != MIR_NO_VALUE && result_type) {
      ...
      char *type_name = type_to_string_dynamic(result_type);
      if (type_name) {
        // Append ": " so the echoed line reads "<Type>: <value>"; for
        // non-printable results (void/fn/module) print the type alone.
        char *label = printable
                          ? mir_arena_printf(program->arena, "%s: ", type_name)
                          : mir_arena_strdup(program->arena, type_name);
        free(type_name);
        if (label) {
          MirValueId type_str = mir_const_string(&builder, &t_string, prog_ast,
                                                 label, strlen(label));
          if (type_str != MIR_NO_VALUE) {
            mir_emit_op1(&builder, MIR_OP_KIND_PRINT, &t_void, prog_ast,
                         type_str);
          }
        }
      }
      if (printable) {
        MirValueId value_str = mir_emit_op1(&builder, MIR_OP_KIND_STR,
                                            &t_string, prog_ast, result);
        if (value_str != MIR_NO_VALUE) {
          mir_emit_op1(&builder, MIR_OP_KIND_PRINT, &t_void, prog_ast,
                       value_str);
        }
      }
      mir_emit_op0(&builder, MIR_OP_KIND_FLUSH, &t_void, prog_ast);
    }
  }

  if (ylc_config.test_mode) {
    mir_build_test_top(program, prog_ast, &build_ctx, &builder);
  } else if (result != MIR_NO_VALUE && builder.block &&
             builder.block->term.kind == MIR_TERM_NONE) {
    mir_builder_set_return(&builder, result);
  } else if (result == MIR_NO_VALUE) {
    mir_builder_set_unreachable_if_open(&builder);
