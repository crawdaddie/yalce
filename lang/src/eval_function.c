...
static Value fn_application(Function func, int num_input, Value *input_vals,
                            ht *stack, int stack_ptr, val_bind_fn_t val_bind) {
  int len = func.len;
  if (num_input == len) {
    ...
    Value res = fn_call(func, input_vals, stack, val_bind);
    // free(input_vals);
    return res;
  }
  ...
  if (num_input + func.num_partial_args == func.len) {
    ...
    // free(input_vals);
    Value res = fn_call(func, func.partial_args, stack, val_bind);
    return res;
  }
...
Value eval_application(Ast *ast, ht *stack, int stack_ptr,
  ...
  if (func_val.type == VALUE_FN && func_val.value.function.is_recursive_ref) {
    // printf("reuse input vals??\n");
    input_vals = func_val.value.function.partial_args;
    // malloc(sizeof(Value) * app_len);
  } else {
  ...
  for (int i = 0; i < app_len; i++) {
    Value arg =
        eval(ast->data.AST_APPLICATION.args + i, stack, stack_ptr, val_bind);
    input_vals[i] = arg;
  }

  if (func_val.type == VALUE_FN) {
    Value res =
        fn_application(func_val.value.function, app_len, input_vals, stack,
                       func_val.value.function.scope_ptr, val_bind);
    // free(input_vals);
    return res;
  }
