...
void advance() { parser->previous = parser->current;
...
static Ast *parse_expression() {
  ...
  Ast *ast = parse_precedence(prefix, PREC_NONE);
  return ast;
}
...
static void push_arg(Ast *fn_application, Ast *arg) {
  if (arg) {
    ...
    fn_application->data.AST_APPLICATION.args = realloc(args, sizeof(Ast *) * len);
