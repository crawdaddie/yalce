...
static void reset_one(reverb_state *r, int inject, const int *combtuning,
                      const int *alltuning) {
  int i;
  memset(r, 0, sizeof(*r));
...
static inline float comb_process_exttrans(comb_state *c, float transval,
...
static void process_one_inner(reverb_state *r, float feedback, float hfdamp,
                              float att, float *input, float *output, long n) {
  float out, val = 0;
  int i;
...
static void process_one_inner2(reverb_state *r, float *transwindow,
                               float feedback1, float feedback2, float hfdamp1,
                               float hfdamp2, float att1, float att2,
                               int inject1, int inject2, float *input1, long n1,
                               float *input2, long n2, float *output) {
  ...
  while (n > 0) {
    ...
    if (inject1 != inject2) {
      for (i = 0; i < numcombs; i++)
        out += comb_process_exttrans(r->comb + i, transwindow[n], feedback,
                                     hfdamp, val);
    } else {
      for (i = 0; i < numcombs; i++)
        out += comb_process(r->comb + i, feedback, hfdamp, val);
...
static void process_one_wrapper(reverb_state *r, float *transwindow,
                                float feedback, float feedback2, float hfdamp,
                                float hfdamp2, float att, float att2,
                                int inject, int inject2, float *input1, long n1,
                                float *input2, long n2, float *output) {
  if (att != att2 || hfdamp != hfdamp2 || feedback != feedback2 ||
      inject != inject2) {

    process_one_inner2(r, transwindow, feedback, feedback2, hfdamp, hfdamp2,
                       att, att2, inject, inject2, input1, n1, input2, n2,
                       output);
  } else {
...
static u_int32_t process_one(reverb_instance_one *ri, reverb_settings *s,
  ...
  if (!ri->initstate) {
    ...
    if (ri->rL)
      reset_one(ri->rL, inject, combL, allL);
    if (ri->rR)
      reset_one(ri->rR, inject, combR, allR);
  ...
  if ((!ri->sC.active && !active) || muted) {
    ...
    if (ri->rL && ri->rL->energy) {
      ...
      process_one_wrapper(ri->rL, transwindow, ri->sC.feedback, feedback,
                          ri->sC.hfdamp, hfdamp, ri->sC.wet, wet, ri->sC.inject,
                          inject, 0, n, 0, 0, outL);

      for (i = 0; i < n; i++)
        if (outL[i] * outL[i] > 1e-15f)
          break;
      if (i == n)
        ri->rL->energy = 0;
      else
    ...
    if (ri->rR && ri->rR->energy) {
      process_one_wrapper(ri->rR, transwindow, ri->sC.feedback, feedback,
                          ri->sC.hfdamp, hfdamp, ri->sC.wet, wet, ri->sC.inject,
                          inject, 0, n, 0, 0, outR);

      for (i = 0; i < n; i++)
        if (outR[i] * outR[i] > 1e-15f)
          break;
      if (i == n)
        ri->rR->energy = 0;
      else
    ...
    if (delay != ri->sC.delay) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (i == first)
          ptr = in;
        cache[i] =
            transwindow[i] * (*ptr++) + (1.f - transwindow[i]) * (*ptrC++);
      }
    ...
    if (ri->rL) {
      ...
      process_one_wrapper(ri->rL, transwindow, ri->sC.feedback, feedback,
                          ri->sC.hfdamp, hfdamp, ri->sC.wet, wet, ri->sC.inject,
                          inject, cache, first, in, second, outL);
    }
    ...
    if (ri->rR) {
      ...
      process_one_wrapper(ri->rR, transwindow, ri->sC.feedback, feedback,
                          ri->sC.hfdamp, hfdamp, ri->sC.wet, wet, ri->sC.inject,
                          inject, cache, first, in, second, outR);
    }
  }
  ...
  /* stereo butterfly? */
  if (ri->rL && ri->rR && energy && (width < 1000 || ri->sC.width < 1000)) {
...
int reverb_process(reverb_instance *ri, reverb_settings **rs, time_linkage *in,
                   time_linkage *outL, time_linkage *outR) {
  ...
  if (ch != outL->channels)
    return -1;
  if (outR && ch != outR->channels)
    return -1;
  if (ri->reverbs->rR && !outR)
    return -1;
  if (!ri->reverbs->rR && outR)
    return -1;
  ...
  case 0:
    ...
    if (in->samples == 0) {
      ...
      if (outL)
        time_linkage_clear(outL);
      if (outR)
        time_linkage_clear(outR);
  ...
  case 1:
    ...
    if (in->samples > 0) {
      for (i = 0; i < ch; i++) {
        ...
        u_int32_t ret = process_one(
            rio, rs[i], in->data[i], mute_channel_muted(in->active, i),
...
time_linkage *p_reverb_read_channel(time_linkage *in, time_linkage **revA,
                                    time_linkage **revB) {
