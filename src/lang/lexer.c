...
static token create_literal_token(token_type type, literal lex_unit) {
  token token;
  token.type = type;
  token.as = lex_unit;
  return token;
...
static token create_symbol_token(token_type type) {
  token token;
  token.type = type;
  return token;
}
...
static int _STRING_MATCHER(const char *input, token *tail) {
  ...
  if (*input == '"') {
    ...
    char *str = (char *)malloc((offset) * sizeof(char));
...
static int _MATCH_IDENTIFIER(const char *input, token *tail) {
  ...
  if ((offset = seek_identifier(input)) != 0) {
    char *str = (char *)malloc((offset + 1) * sizeof(char));
