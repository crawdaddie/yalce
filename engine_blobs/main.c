...
blob_state *compile_blob_def(void **mem) {
  blob_state *blob = malloc(sizeof(blob_state));
  *blob = (blob_state){.num_ins = 0};
  Signal *freq = create_const_sig(mem, 100.);
  blob->num_ins++;

  Node *sin_node = create_sin_node(mem, freq);
  blob->blob_start = sin_node;
  blob->total_size += sin_node->node_size;

  Node *tanh_node = create_tanh_node(mem, &(sin_node->out));
  blob->total_size += tanh_node->node_size;
  blob->blob_end = tanh_node;
  return blob;
}
